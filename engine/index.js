export { GameLoop } from './core/GameLoop.js';
export { Vector2 } from './core/Vector2.js';
export { EventBus } from './core/EventBus.js';
export { StateMachine } from './core/StateMachine.js';
export { MathUtils } from './core/MathUtils.js';

export { ProceduralPrimitives } from './rendering/ProceduralPrimitives.js';
export { CanvasRenderer } from './rendering/CanvasRenderer.js';
export { CharacterComposer } from './characters/CharacterComposer.js';

export { Entity } from './entities/Entity.js';
export { Vehicle } from './entities/Vehicle.js';
export { Collectible } from './entities/Collectible.js';
export { Lane } from './entities/Lane.js';
export { Enemy } from './entities/Enemy.js';

export { Interactable } from './interactions/Interactable.js';
export { NPC } from './interactions/NPC.js';
export { DialogueBox } from './interactions/DialogueBox.js';
export { AbilityGate } from './interactions/AbilityGate.js';
export { Checkpoint } from './interactions/Checkpoint.js';

export { TweenManager, Easings } from './animation/TweenManager.js';
export { ParticleSystem } from './particles/ParticleSystem.js';
export { JuiceEffects } from './effects/JuiceEffects.js';
export { ProceduralAudio } from './audio/ProceduralAudio.js';
export { InputManager } from './input/InputManager.js';
export { Camera2D } from './camera/Camera2D.js';
export { PlaygamaBridge } from './platform/playgama/PlaygamaBridge.js';
